...
- [ ] T008 Add Prisma to `backend/`; define `backend/prisma/schema.prisma` with `Anime`, `Genre`, `AnimeGenre` models + enums (`GenreKind`, `ContentRating`) per data-model.md
- [ ] T009 Generate the initial Prisma migration and a `PrismaService` in `backend/src/common/prisma.service.ts`
- [ ] T010 [P] Define shared Zod schemas mirroring `contracts/openapi.yaml` in `backend/src/common/schemas.ts` (TasteProfile, Recommendation, Genre, AnimeSummary, Error)
- [ ] T011 [P] Add AJV validators for raw Jikan responses in `backend/src/catalog/ingestion/jikan.schema.ts` (untrusted external input)
...
- [ ] T018 [P] [US1] Implement pure scoring in `backend/src/recommendations/scoring.ts` (candidate set + weighted score + top-25 shortlist per data-model.md)
- [ ] T019 [P] [US1] Implement banding in `backend/src/recommendations/banding.ts` (score → CHEFS_PICK / RECOMMENDED / WORTH_A_TRY)
- [ ] T020 [US1] Unit test the scoring + banding in `backend/test/scoring.spec.ts` (Vitest) — covers genre overlap, favorite similarity, exclusions, thresholds
